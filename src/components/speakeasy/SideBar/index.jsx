...
const sidebarContentAtom = atom(null);
export function SideBar() {
    ...
    const clickShield = useCallback((e) => {
        e.stopPropagation();
        e.preventDefault();
    }, []);
...
export function SideBarCta({ cta, children, title, }) {
    const [, setContent] = useAtom(sidebarContentAtom);
    const onClick = useCallback(() => setContent({ title, content: children }), [title, children]);
